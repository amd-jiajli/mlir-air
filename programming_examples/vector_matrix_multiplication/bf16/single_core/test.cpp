...
void add_default_options(cxxopts::Options &options) {
  options.add_options()("help,h", "produce help message")(
      ...
      cxxopts::value<int>()->default_value("288"))(
      "size_n,N", "Matrix width N (output dimension)",
...
int main(int argc, const char *argv[]) {
  ...
  cxxopts::Options options("GEMV BF16 Profiling");
  ...
  // Initialize input vector A with random bfloat16 values
  ...
  // Initialize input matrix B with random bfloat16 values
  ...
  // GEMV: C[N] = A[K] * B[K, N], MACs = 2 * K * N
  float macs = 2.0 * float(K) * float(N);
  ...
  std::cout << std::endl
            << "GEMV size: K=" << K << ", N=" << N << std::endl;

  std::cout << std::endl
            << "Avg NPU GEMV time: " << npu_time_total / n_iterations << "us."
            << std::endl;
  std::cout << "Avg NPU gflops: "
            << macs / (1000 * npu_time_total / n_iterations) << std::endl;

  std::cout << std::endl
            << "Min NPU GEMV time: " << npu_time_min << "us." << std::endl;
  std::cout << "Max NPU gflops: " << macs / (1000 * npu_time_min) << std::endl;

  std::cout << std::endl
            << "Max NPU GEMV time: " << npu_time_max << "us." << std::endl;
